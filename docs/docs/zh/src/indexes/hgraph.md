...
`Merge` 会保留每个输入索引中已有的重复组，以及组内共享的物理编码槽位。它不会在目标索引与
源索引之间或多个源索引之间重新判重。因此，不同输入索引中的相同向量仍属于不同的重复组，
并使用各自的物理编码槽位。目标索引和所有源索引必须使用兼容的索引参数；使用需要训练的
量化器时，还必须来自同一个导出模型。RaBitQ 未配置 precise reorder 时，还必须设置
`store_raw_vector: true`，以便重建图。所有输入索引都不能包含已标记删除的向量。
