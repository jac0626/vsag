...
make_build_param(bool support_duplicate,
                 float duplicate_threshold = 0.0F,
...
test_vectors
generate_test_data(int64_t dim, int64_t base_count, int64_t duplicate_count, uint32_t seed = 42) {
...
vsag::IndexPtr
build_index_with_duplicates(const test_vectors& vectors,
                            const std::string& build_param,
                            vsag::Allocator* allocator = nullptr) {
    auto index = vsag::Factory::CreateIndex("hgraph", build_param, allocator);
...
class PatternAllocator : public vsag::DefaultAllocator {
public:
    void*
    Reallocate(void* pointer, uint64_t size) override {
        ...
        const auto old_size = sizes_.at(pointer);
        auto* data = static_cast<uint8_t*>(vsag::DefaultAllocator::Reallocate(pointer, size));
        sizes_.erase(pointer);
        sizes_[data] = size;
        if (size > old_size) {
            fill(data, old_size, size);
        }
        return data;
    }
    ...
    Allocate(uint64_t size) override {
        auto* data = static_cast<uint8_t*>(vsag::DefaultAllocator::Allocate(size));
        // Unwritten graph rows must not be interpreted as adjacency lists. Seed fresh
        // memory with a small valid count followed by ID 0 to expose accidental reads.
        fill(data, 0, size);
        std::lock_guard<std::mutex> lock(mutex_);
        sizes_[data] = size;
        return data;
    ...
    Deallocate(void* pointer) override {
        std::lock_guard<std::mutex> lock(mutex_);
        sizes_.erase(pointer);
        vsag::DefaultAllocator::Deallocate(pointer);
    }

private:
    static void
    fill(uint8_t* data, uint64_t begin, uint64_t end) {
        for (uint64_t offset = begin; offset < end; ++offset) {
            data[offset] = offset % sizeof(uint64_t) == 0 ? 1 : 0;
        }
    }

    std::mutex mutex_;
    std::map<void*, uint64_t> sizes_;
};
...
collect_iterator_results(const vsag::IndexPtr& index,
    ...
    iterator_context_guard guard{iterator_context};
...
TEST_CASE("HGraph degree statistics skip duplicate adjacency",
          "[ft][hgraph][duplicate][analyzer]") {
    PatternAllocator allocator;
    auto vectors = generate_test_data(DIM, 2, 6);
    auto build_param = nlohmann::json::parse(make_build_param(true, 0.001F));
    build_param["index_param"]["graph_io_type"] = "memory_io";
    auto index = build_index_with_duplicates(vectors, build_param.dump(), &allocator);
    const auto stats = nlohmann::json::parse(index->GetStats());
    const auto in_degrees = stats.at("in_degree_distribution").get<std::vector<uint32_t>>();
    const auto out_degrees = stats.at("out_degree_distribution").get<std::vector<uint32_t>>();
    uint64_t incoming = 0;
    uint64_t outgoing = 0;
    for (uint64_t degree = 0; degree < in_degrees.size(); ++degree) {
        incoming += degree * in_degrees[degree];
        outgoing += degree * out_degrees[degree];
    }
    REQUIRE(outgoing == 2);
...
TEST_CASE("HGraph dedup search expands the entry-point group",
    ...
    auto vectors = generate_test_data(DIM, entry_base_count, entry_duplicate_count);
    const auto* quantization = GENERATE("fp32", "sq8");
    auto index = build_index_with_duplicates(vectors, make_build_param(true, 0.001F, quantization));
    ...
    REQUIRE(count_duplicate_ids(default_limit.value(), entry_base_count) == entry_duplicate_count);
}
...
TEST_CASE("HGraph dedup search limits duplicate expansion per group",
    ...
    auto vectors = generate_test_data(DIM, multi_base_count, multi_duplicate_count);
    auto index = build_index_with_duplicates(vectors, make_build_param(true, 0.001F));

    auto query = vsag::Dataset::Make();
    query->NumElements(1)->Dim(DIM)->Float32Vectors(vectors.queries.data())->Owner(false);

    auto result = index->KnnSearch(query, 4, make_search_param(8, 1));
    REQUIRE(result.has_value());
    REQUIRE(result.value()->GetDim() == 4);
    const auto group_duplicate_count =
        count_duplicate_ids_by_group(result.value(), multi_base_count);
...
TEST_CASE("HGraph dedup search supports zero and unlimited duplicate expansion",
    ...
    auto vectors = generate_test_data(DIM, base_count, duplicate_count);
    auto index = build_index_with_duplicates(vectors, make_build_param(true, 0.001F));

    auto query = vsag::Dataset::Make();
    query->NumElements(1)->Dim(DIM)->Float32Vectors(vectors.queries.data())->Owner(false);

    auto limited = index->KnnSearch(query, 8, make_search_param(8, 0));
    REQUIRE(limited.has_value());
    REQUIRE(limited.value()->GetDim() == base_count);
    REQUIRE(count_duplicate_ids(limited.value(), base_count) == 0);

    auto unlimited = index->KnnSearch(query, 8, make_search_param(8, -1));
    REQUIRE(unlimited.has_value());
    REQUIRE(unlimited.value()->GetDim() == base_count + duplicate_count);
    REQUIRE(count_duplicate_ids(unlimited.value(), base_count) == duplicate_count);
}
...
TEST_CASE("HGraph dedup search ignores duplicate limit when tracking is disabled",
    ...
    constexpr int64_t duplicate_count = 6;
    auto vectors = generate_test_data(DIM, base_count, duplicate_count);
    auto index = build_index_with_duplicates(vectors, make_build_param(false));

    auto query = vsag::Dataset::Make();
    query->NumElements(1)->Dim(DIM)->Float32Vectors(vectors.queries.data())->Owner(false);

    auto limited = index->KnnSearch(query, 8, make_search_param(16, 0));
    auto unlimited = index->KnnSearch(query, 8, make_search_param(16, -1));
    REQUIRE(limited.has_value());
    REQUIRE(unlimited.has_value());
...
TEST_CASE("HGraph dedup search applies filters before counting duplicate limits",
    ...
    constexpr int64_t base_count = 2;
    auto vectors = generate_test_data(DIM, base_count, 3 * base_count);
    auto index = build_index_with_duplicates(vectors, make_build_param(true, 0.001F));
        ...
        DYNAMIC_SECTION("parallelism=" << parallelism) {
            for (int64_t group = 0; group < base_count; ++group) {
                for (int64_t member = 0; member < 3; ++member) {
                    ...
                    auto result =
                        index->KnnSearch(query, 1, make_search_param(8, 1, parallelism), filter);
                    REQUIRE(result.has_value());
...
TEST_CASE("HGraph duplicate limit does not change range-search expansion",
    ...
    constexpr int64_t base_count = 20;
    constexpr int64_t duplicate_count = 60;
    auto vectors = generate_test_data(DIM, base_count, duplicate_count);
    auto index = build_index_with_duplicates(vectors, make_build_param(true, 0.001F));
    auto query = vsag::Dataset::Make();
    query->NumElements(1)->Dim(DIM)->Float32Vectors(vectors.queries.data())->Owner(false);
    ...
    for (const auto parallelism : {1, 2}) {
        DYNAMIC_SECTION("parallelism=" << parallelism) {
            auto result = index->RangeSearch(
                query, 100.0F, make_search_param(200, 0, parallelism), filter, -1);
            REQUIRE(result.has_value());
            auto unlimited = index->RangeSearch(
                query, 100.0F, make_search_param(200, -1, parallelism), filter, -1);
            REQUIRE(unlimited.has_value());
            REQUIRE(count_duplicate_ids(unlimited.value(), base_count) > 0);
            const std::set<int64_t> limited_ids(
                result.value()->GetIds(), result.value()->GetIds() + result.value()->GetDim());
            const std::set<int64_t> unlimited_ids(
                unlimited.value()->GetIds(),
                unlimited.value()->GetIds() + unlimited.value()->GetDim());
...
TEST_CASE("HGraph dedup iterator keeps limited duplicates across pages",
    ...
    auto vectors = generate_test_data(DIM, iterator_base_count, iterator_duplicate_count);
    auto index = build_index_with_duplicates(vectors, make_build_param(true, 0.001F));
    auto query = vsag::Dataset::Make();
    query->NumElements(1)->Dim(DIM)->Float32Vectors(vectors.queries.data())->Owner(false);
...
TEST_CASE("HGraph dedup iterator filters duplicate groups before applying limits",
    ...
    for (const auto allowed_duplicate : vectors.duplicate_ids) {
        DYNAMIC_SECTION("allowed_duplicate=" << allowed_duplicate) {
            ...
            const auto items =
                collect_iterator_results(index, query, make_search_param(4, 1), 1, filter);
...
TEST_CASE("HGraph dedup iterator drains pending duplicates in last-search mode",
          "[ft][hgraph][duplicate][search_control][iterator]") {
    constexpr int64_t iterator_base_count = 2;
    auto vectors = generate_test_data(DIM, iterator_base_count, 6);
    auto index = build_index_with_duplicates(vectors, make_build_param(true, 0.001F));
    auto query = vsag::Dataset::Make();
    query->NumElements(1)->Dim(DIM)->Float32Vectors(vectors.queries.data())->Owner(false);
