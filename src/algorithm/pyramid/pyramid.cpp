...
std::vector<int64_t>
Pyramid::prepare_build_data(const DatasetPtr& base, Vector<int64_t>& data_biases) {
    ...
    resize(std::max(INIT_CAPACITY, data_num));
    data_biases.reserve(data_num);
    for (int64_t i = 0; i < data_num; ++i) {
        if (label_table_->CheckLabel(data_ids[i])) {
            logger::warn("Label {} already exists, skip adding.", data_ids[i]);
            failed_ids.push_back(data_ids[i]);
            continue;
        }
        const auto inner_id = static_cast<InnerIdType>(data_biases.size());
        label_table_->Insert(inner_id, data_ids[i]);
        data_biases.push_back(i);
    }

    batch_insert_build_codes(base_codes_, data_vectors, data_biases);
    if (use_reorder_) {
        batch_insert_build_codes(precise_codes_, data_vectors, data_biases);
    }
    return failed_ids;
}

void
Pyramid::batch_insert_build_codes(const FlattenInterfacePtr& codes,
    ...
    const uint64_t code_size = std::max<uint64_t>(1, codes->code_size_);
    const uint64_t batch_size = std::max<uint64_t>(1, BUILD_CODES_BATCH_BYTES / code_size);
    uint64_t run_begin = 0;
    while (run_begin < data_biases.size()) {
        uint64_t run_end = run_begin + 1;
        while (run_end < data_biases.size() &&
               data_biases[run_end] == data_biases[run_end - 1] + 1) {
            ++run_end;
        }
        for (uint64_t offset = run_begin; offset < run_end; offset += batch_size) {
            const uint64_t count = std::min(batch_size, run_end - offset);
            const auto data_bias = data_biases[offset];
            codes->BatchInsertVector(data_vectors + dim_ * data_bias,
...
void
Pyramid::collect_nsw_build_jobs(Hierarchy& hierarchy,
                                IndexNode* node,
                                Vector<NswBuildJob>& build_jobs) {
    const bool skip_index =
        std::find(hierarchy.no_build_levels.begin(),
                  hierarchy.no_build_levels.end(),
                  static_cast<int32_t>(node->level_)) != hierarchy.no_build_levels.end();
    if (skip_index) {
        node->status_ = IndexNode::Status::NO_INDEX;
        Vector<InnerIdType>(node->allocator_).swap(node->ids_);
    } else if (not node->ids_.empty()) {
        node->Init();
        if (node->status_ == IndexNode::Status::GRAPH) {
            build_jobs.push_back({&hierarchy, node});
        }
    }

    for (auto& [key, child] : node->children_) {
        (void)key;
        collect_nsw_build_jobs(hierarchy, child.get(), build_jobs);
    }
}
...
std::vector<int64_t>
Pyramid::build_by_nsw(const DatasetPtr& base) {
    const auto* data_vectors = base->GetFloat32Vectors();
    ...
    Vector<NswBuildJob> build_jobs(allocator_);
    for (auto& [hname, hierarchy] : hierarchies_) {
        (void)hname;
    ...
    for (const auto& job : build_jobs) {
        Vector<InnerIdType>(allocator_).swap(job.node->ids_);
    }
...
Pyramid::Build(const DatasetPtr& base) {
    CHECK_ARGUMENT(base_codes_->TotalCount() == 0, "index is not empty");
    int64_t data_num = base->GetNumElements();

    this->Train(base);
    if (graph_type_ == GRAPH_TYPE_VALUE_NSW) {
        return this->build_by_nsw(base);
    }
