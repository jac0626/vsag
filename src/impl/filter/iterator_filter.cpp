...
IteratorFilterContext::AddDiscardNode(float dis, uint32_t inner_id) {
    if (this->IsPendingDuplicate(inner_id)) {
        // Keep the bottom-graph search distance stored in pending_duplicates_. A reorder
        // eviction may carry a refined distance that must not affect the next graph search's
        // lower bound.
        return;
    }
