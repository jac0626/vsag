...
function (vsag_link_openmp_private target)
    if (NOT ENABLE_OPENMP OR NOT target)
        return ()
    endif ()
...
function (vsag_enable_openmp_private target)
    if (NOT ENABLE_OPENMP OR NOT target OR NOT TARGET "${target}")
        return ()
    endif ()
    if (TARGET vsag_openmp)
        target_link_libraries ("${target}" PRIVATE vsag_openmp)
    else ()
        target_compile_definitions ("${target}" PRIVATE VSAG_ENABLE_OPENMP=1)
        target_compile_options ("${target}" PRIVATE -fopenmp)
        target_link_options ("${target}" PRIVATE -fopenmp)
    endif ()
endfunction ()

function (vsag_enable_openmp_interface target)
    if (NOT ENABLE_OPENMP OR NOT target OR NOT TARGET "${target}")
        return ()
    endif ()
    if (TARGET vsag_openmp)
        target_link_libraries ("${target}" INTERFACE vsag_openmp)
    else ()
        target_compile_definitions ("${target}" INTERFACE VSAG_ENABLE_OPENMP=1)
        target_compile_options ("${target}" INTERFACE -fopenmp)
        target_link_options ("${target}" INTERFACE -fopenmp)
    endif ()
endfunction ()
